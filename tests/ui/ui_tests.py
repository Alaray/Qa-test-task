...
def test_insider_qa_jobs(browser):
  try:
    ...
    logger.info("Step 1: Insider home page is opened.")
    ...
    logger.info("Step 2: Career page and its blocks are opened.")
    ...
    logger.info("Step 3: QA jobs filtered by Location - Istanbul, Turkey and department - Quality Assurance. Jobs list is present.")

    # Step 4
    all_jobs = jobs_page.get_all_jobs()
    for job in all_jobs:
      jobs_page.check_job_details(job)
    logger.info("Step 4: All jobs' Position, Department, and Location contain the expected values.")
    ...
    logger.info("Step 5: clicking 'View Role' redirected to Lever Application page.")
